 ...
 *
 * UI tests for FTS SQL. They drive a real browser against a real Nextcloud,
...
test('the app page renders its template', async ({ page }) => {
	await page.goto('apps/fts_sql/')

	await expect(page.getByRole('heading', { name: 'FTS SQL' })).toBeVisible()
...
test('the app appears in the navigation', async ({ page }) => {
	...
	const entry = page.getByRole('menuitem', { name: 'FTS SQL' })
		.or(page.getByRole('link', { name: 'FTS SQL' }))
...
test('the admin settings section renders and saves', async ({ page }) => {
	await page.goto('settings/admin/fts_sql')
...
test('the data API stores and returns an item', async ({ page }) => {
	...
	const created = await page.evaluate(async (t) => {
		const response = await fetch(OC.generateUrl('/apps/fts_sql/api/items'), {
			method: 'POST',
